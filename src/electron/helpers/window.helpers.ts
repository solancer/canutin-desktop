...
export const calculateWindowWidth = (displayWidth: number) => {
  ...
  const relativeWindowWidth = Math.floor(
    isDev ? displayWidth * 0.7 + DEV_TOOLS_PANEL_WIDTH : displayWidth * 0.7
  );
...
export const calculateWindowHeight = (displayHeight: number) => {
  const relativeWindowHeight = Math.floor(displayHeight * 0.8);
